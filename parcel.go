...
func (s ParcelStore) Get(number int) (Parcel, error) {
	...
	var p Parcel
	err := parselRow.Scan(&p.Number, &p.Client, &p.Status, &p.Address, &p.CreatedAt)
	if err != nil {
		return Parcel{}, err
	}
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	for parcelRows.Next() {
		...
		var p Parcel
		err := parcelRows.Scan(&p.Number, &p.Client, &p.Status, &p.Address, &p.CreatedAt)
		if err != nil {
			return nil, err
		}
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	res, err := s.db.Exec(
		"UPDATE parcel SET address = :addres WHERE number = :number AND status = :statusregistered",
		sql.Named("addres", address), sql.Named("number", number), sql.Named("statusregistered", ParcelStatusRegistered),
	)
	if err != nil {
		return err
	}
...
func (s ParcelStore) Delete(number int) error {
	...
	_, err := s.db.Exec(
		"DELETE FROM parcel WHERE number = :number AND status = :statusregistered",
		sql.Named("number", number), sql.Named("statusregistered", ParcelStatusRegistered),
	)
